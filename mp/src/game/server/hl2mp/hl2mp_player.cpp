...
ConVar cl_giblife("cl_giblife", "20", FCVAR_ARCHIVE, "Set how much time the gibs are going to stay before they fade out");
...
void CHL2MP_Player::Precache( void )
{
	...
	PrecacheModel("models/char/rif/rifleman2.mdl");
	PrecacheModel("models/char/gibs/rif_gibs.1.mdl"); 
	PrecacheModel("models/char/gibs/rif_gibs.2.mdl");
	PrecacheModel("models/char/gibs/rif_gibs.3.mdl");
	PrecacheModel("models/char/gibs/rif_gibs.5.mdl");
	PrecacheModel("models/char/gibs/rif_gibs.6.mdl");
	PrecacheModel("models/char/gibs/rif_gibs.7.mdl");
	PrecacheModel("models/char/gibs/rif_gibs.8.mdl");
}
...
//	int modelIndex = modelinfo->GetModelIndex( szModelName );
	...
	//{
	//	szModelName = "models/char/rif/rifleman2.mdl";
//		m_iModelType = TEAM_COMBINE;
	...
	if ( GetTeamNumber() == TEAM_COMBINE )
	{
		...
		SetModel("models/char/rif/rifleman2.mdl");
		SetupPlayerSoundsByModel("models/char/rif/rifleman2.mdl");
		m_nSkin = 1;
		m_iModelType = TEAM_COMBINE;
	...
	else if ( GetTeamNumber() == TEAM_REBELS )
	{
		...
		SetModel("models/char/rif/rifleman.mdl");
		SetupPlayerSoundsByModel("models/char/rif/rifleman.mdl");
		m_nSkin = 0;
...
void CHL2MP_Player::SetPlayerModel( void )
{
	...
	if ( ValidatePlayerModel( szModelName ) == false )
	{
		...
		{
			pszCurrentModelName = "models/char/rif/rifleman.mdl";
		}
	...
	else
	{
		...
		if (Q_stristr(szModelName, "models/char/rif/rifleman.mdl"))
		{
			m_iModelType = TEAM_REBELS;
			m_nSkin = 0;
		}
...
{
	if (Q_stristr(pModelName, "models/char/rif/rifleman.mdl"))
	{
		m_iPlayerSoundType = (int)PLAYER_SOUNDS_CITIZEN;
	}
	else if ( Q_stristr(pModelName, "models/char/rif/rifleman2.mdl" ) )
	{
...
void CHL2MP_Player::CreateRagdollEntity( void )
{
	...
	if ( pRagdoll )
	{
		...
		pRagdoll->m_nSkin = GetSkin(); // Quick hack to make the skin actually change. I'll explain later
...
void CHL2MP_Player::Event_Killed(const CTakeDamageInfo &info)
{
	...
	int m_RandomGib = random->RandomInt(1, 4);
	...
	// Quick hunk of junk code that makes a very basic gibbing effect.
	// When the player gets killed by a big explosion, i.e a bomb or an artillery shell, we just tear them to little pieces.
	// We remove the ragdoll corpse and we spawn a few gib props to give the illusion that the player's corpse is FUBAR.

	if (info.GetDamageType() & (DMG_BLAST) && (info.GetDamage() < (m_iMaxHealth * 0.75f)) || info.GetDamageType() & (DMG_BUCKSHOT) && (info.GetDamage() > (m_iMaxHealth * 0.50f))) {

		CBaseEntity *pLegsGib = CreateRagGib("models/char/rif/rifleman2.mdl", GetAbsOrigin(), GetAbsAngles(), vecDamageDir, 20.0f);
		if (pLegsGib)	{ 
			
			if (GetTeamNumber() == TEAM_COMBINE)
			{
				pLegsGib->GetBaseAnimating()->m_nSkin = 1;
			}
			...
			if (m_RandomGib == 1)
			{
				...
				CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.5.mdl", cl_giblife.GetFloat()); // arm
				CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.6.mdl", cl_giblife.GetFloat()); // leg
				CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.8.mdl", cl_giblife.GetFloat()); // leg 2
				
			pLegsGib->GetBaseAnimating()->SetBodygroup(1, 1); //face
			pLegsGib->GetBaseAnimating()->SetBodygroup(3, 1); //rarm
			pLegsGib->GetBaseAnimating()->SetBodygroup(4, 1); //rleg
			pLegsGib->GetBaseAnimating()->SetBodygroup(5, 1); //lleg
			pLegsGib->GetBaseAnimating()->SetBodygroup(6, 0); //larm
            UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_RED, 15, FX_BLOODSPRAY_ALL);
			}

			else if (m_RandomGib == 2)
			{



				CGib::SpawnSpecificGibs(this, 2, 150, 300, "models/char/gibs/rif_gibs.5.mdl", cl_giblife.GetFloat()); // arm
				CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.6.mdl", cl_giblife.GetFloat()); // leg
				CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.8.mdl", cl_giblife.GetFloat()); // leg 2
				...
				pLegsGib->GetBaseAnimating()->SetBodygroup(4, 1); //rleg
				pLegsGib->GetBaseAnimating()->SetBodygroup(5, 1); //lleg
				pLegsGib->GetBaseAnimating()->SetBodygroup(6, 1); //larm
				UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_RED, 15, FX_BLOODSPRAY_ALL);
			...
			else if (m_RandomGib == 3)
			{
				...
				pLegsGib->GetBaseAnimating()->SetBodygroup(1, 1); //face
				pLegsGib->GetBaseAnimating()->SetBodygroup(3, 1); //rarm
				pLegsGib->GetBaseAnimating()->SetBodygroup(4, 0); //rleg
				pLegsGib->GetBaseAnimating()->SetBodygroup(5, 1); //lleg
				pLegsGib->GetBaseAnimating()->SetBodygroup(6, 0); //larm
				UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_RED, 15, FX_BLOODSPRAY_ALL);
			}
			...
			else if (m_RandomGib == 4)
			{
				...
				pLegsGib->GetBaseAnimating()->SetBodygroup(1, 1); //face
				pLegsGib->GetBaseAnimating()->SetBodygroup(3, 0); //rarm
				pLegsGib->GetBaseAnimating()->SetBodygroup(4, 1); //rleg
				pLegsGib->GetBaseAnimating()->SetBodygroup(5, 1); //lleg
				pLegsGib->GetBaseAnimating()->SetBodygroup(6, 0); //larm
				UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_RED, 15, FX_BLOODSPRAY_ALL);
		...
		if (info.GetDamageType() & (DMG_BLAST) && info.GetDamage() >= (m_iMaxHealth * 0.75f))
		{
			...
			CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.1.mdl", cl_giblife.GetFloat()); // upper body
			CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.2.mdl", cl_giblife.GetFloat()); // lower body
			CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.3.mdl", cl_giblife.GetFloat()); // head piece 
			CGib::SpawnSpecificGibs(this, 2, 150, 300, "models/char/gibs/rif_gibs.5.mdl", cl_giblife.GetFloat()); // arm
			CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.6.mdl", cl_giblife.GetFloat()); // leg
			CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.7.mdl", cl_giblife.GetFloat()); // helmet
			CGib::SpawnSpecificGibs(this, 1, 150, 300, "models/char/gibs/rif_gibs.8.mdl", cl_giblife.GetFloat()); // leg 2
			UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_RED, 15, FX_BLOODSPRAY_ALL);

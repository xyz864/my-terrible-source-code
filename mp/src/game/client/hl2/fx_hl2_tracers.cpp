...
void FX_PlayerAR2Tracer( const Vector &start, const Vector &end )
{
	...
	//Create the line
	CFXStaticLine *tracerLine = new CFXStaticLine( "Tracer", dStart, dEnd, random->RandomFloat( 10.0f, 18.0f ), 0.01f, "effects/gunshiptracer", 0 );
	assert( tracerLine );
...
void FX_AR2Tracer( Vector& start, Vector& end, int velocity, bool makeWhiz )
{
	...
	float length = random->RandomFloat( 100.0f, 156.0f);
	float life = ( dist + length ) / velocity;	//NOTENOTE: We want the tail to finish its run as well
	
	//Add it
	FX_AddDiscreetLine( start, dir, velocity, length, dist, random->RandomFloat( 2.5f, 3.5f ), life, "effects/gunshiptracer" );
